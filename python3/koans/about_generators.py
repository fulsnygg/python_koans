...
class AboutGenerators(Koan):
    ...
    def test_generator_keeps_track_of_local_variables(self):
        ...
        result2 = self.sum_it(range(1,3))
        self.assertEqual([1, 3], list(result2))
    ...
    def test_generators_can_act_as_coroutines(self):
        ...
        #next(generator)
        generator.send(None) # This works too!
        # Generators start executing at top of generator's function body,
        # therefore the yield expression does not exist when the generator has
        # just been created. We must first use next() method or send(None) to
        # initiate the yield, then it can be assigned a value by send() method
    ...
    def test_generators_can_see_if_they_have_been_called_with_a_value(self):
        ...
        generator3 = self.yield_tester()
        next(generator3)
        self.assertEqual('no value', generator3.send(None)) # also works
        ...
        try:
            ...
            self.assertEqual({}, ex.__dict__)
            self.assertEqual(ex.value, None)

        # Or simpler maybe..:
        with self.assertRaises(StopIteration): next(generator3)
