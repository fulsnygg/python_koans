...
class AboutClasses(Koan):
    ...
    def test_you_can_also_access_the_value_out_using_getattr_and_dict(self):
        ...
        self.assertEqual("Fido", fido._name) # through assignment operator
    ...
    def test_that_name_can_be_read_as_a_property(self):
        ...
        with self.assertRaises(AttributeError): fido.name

        try:
            fido.name
        except AttributeError as ex:
            self.assertRegex("'Dog3' object has no attribute '_name'", ex.args[0])
    ...
    def test_init_provides_initial_values_for_instance_variables(self):
        ...
        # Gives NameError
        #dogo = self.Dog5(Dogo)
        #self.assertEqual(Dogo, dogo.name)
    ...
    def test_different_objects_have_different_instance_variables(self):
        ...
        self.assertEqual(0, rover.name == fido.name)
